...
import { Trophy, Clock, Crown, TrendingUp, Calendar, Award, Shield, Target, Users } from 'lucide-react';
import { Link } from 'react-router-dom';
import { createPageUrl } from '@/utils';
import { Badge } from "@/components/ui/badge";
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from "@/components/ui/table";

export default function LeagueHistoricalStats({ seasons, leagueTables, clubs, league }) {
    const stats = useMemo(() => {
        ...
        // Clubs by total seasons in this league
        const clubSeasonCounts = {};
        leagueTables.forEach(table => {
            if (!table.club_name) return;
            const name = table.club_name.trim();
            clubSeasonCounts[name] = (clubSeasonCounts[name] || 0) + 1;
        });
        const longestServingClubs = Object.entries(clubSeasonCounts)
            .sort((a, b) => b[1] - a[1])
            .slice(0, 10)
            .map(([name, seasons]) => ({
                name,
                seasons,
                percentage: Math.round((seasons / leagueTables.filter(t => t.club_name).length * clubSeasonCounts[name]) * 100)
            }));

        // Most appearances without winning
        const neverWonButAppeared = Object.entries(clubSeasonCounts)
            .filter(([name, count]) => {
                const hasTitle = seasons.some(s => s.champion_name?.trim() === name);
                return !hasTitle && count >= 5;
            })
            .sort((a, b) => b[1] - a[1])
            .slice(0, 5);
        ...
        // Longest title drought for clubs that won before
        const clubLastTitle = {};
        seasons.sort((a, b) => a.year.localeCompare(b.year)).forEach(season => {
            if (season.champion_name) {
                clubLastTitle[season.champion_name.trim()] = season.year;
            }
        });

        // Title defenses (back-to-back)
        const titleDefenses = [];
        ...
        // One-time champions (won exactly once)
        const titleCounts = {};
        seasons.forEach(s => {
            if (s.champion_name) {
    ...
    return (
        <div className="space-y-6">
                                ...
                                <div key={decade} className="p-4 rounded-lg bg-gradient-to-r from-purple-50 to-pink-50 border border-purple-200">
                                    <div className="flex items-center justify-between mb-2">
                                        <div className="flex items-center gap-3">
                                            ...
                                            <div className="text-slate-500 text-sm">({totalSeasons} season{totalSeasons > 1 ? 's' : ''})</div>
                                        </div>
                                        <Badge className="bg-purple-500">
                                            {uniqueChampions} different champion{uniqueChampions > 1 ? 's' : ''}
                                        </Badge>
                                    </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                {stats.longestServingClubs.length > 0 && (
                    <Card className="border-0 shadow-sm">
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                                <Clock className="w-5 h-5 text-blue-500" />
                                Most Seasons in Division
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            <Table>
                                ...
                                <TableBody>
                                    {stats.longestServingClubs.map(({ name, seasons }, idx) => {
                                        ...
                                        return (
                                            <TableRow key={name}>
                                                ...
                                                <TableCell className="text-center">
                                                    <span className="font-bold text-blue-600">{seasons}</span>
                                                </TableCell>
                                            </TableRow>
                                        );
                                    })}
                ...
                {stats.mostTitleDefenses.length > 0 && (
                    <Card className="border-0 shadow-sm">
                        ...
                        <CardContent>
                            <div className="space-y-3">
                                {stats.mostTitleDefenses.map(({ club, count, streakYears }, idx) => {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                {/* Never Won But Loyal */}
                {stats.neverWonButAppeared.length > 0 && (
                    <Card className="border-0 shadow-sm">
                        <CardHeader>
                            <CardTitle className="flex items-center gap-2">
                                <Shield className="w-5 h-5 text-slate-500" />
                                Loyal Clubs (Never Won Title)
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            <Table>
                                ...
                                <TableBody>
                                    {stats.neverWonButAppeared.map(([name, count]) => {
                                        const club = clubs.find(c => c.name === name);
                                        return (
                                            <TableRow key={name}>
                                                <TableCell className="font-medium">
                                                ...
                                                <TableCell className="text-center text-slate-600">{count}</TableCell>
                                            </TableRow>
                                        );
                                    })}
                                </TableBody>
                            ...
                            <p className="text-xs text-slate-500 mt-3 italic">
                                Clubs that have been mainstays but never lifted the trophy
                            </p>
                        </CardContent>
                    </Card>
                )}
                ...
                <Card className="border-0 shadow-sm">
                    ...
                    <CardContent>
                        <div className="flex flex-wrap gap-2">
                            {stats.oneTimeChampions.map(({ name, year }) => {
                                const club = clubs.find(c => c.name === name);
                                return (
                        ...
                        <p className="text-xs text-slate-500 mt-3 italic">
                            Clubs that won the title exactly once - their moment of glory
                        </p>
                    </CardContent>
                </Card>
            )}
                ...
                <CardContent className="p-6">
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
                        <div>
                            ...
                            <div className="text-3xl font-bold text-slate-700">{stats.uniqueChampions}</div>
                        ...
                        <div>
                            ...
                            <div className="text-sm text-slate-500">Total Seasons</div>
                        </div>
                        <div>
                            <Shield className="w-6 h-6 text-blue-500 mx-auto mb-2" />
                            <div className="text-3xl font-bold text-blue-600">{stats.longestServingClubs.length > 0 ? stats.longestServingClubs[0].seasons : 0}</div>
                            <div className="text-sm text-slate-500">Most Seasons (1 club)</div>
                        </div>
                        <div>
                            <Award className="w-6 h-6 text-emerald-500 mx-auto mb-2" />
                            <div className="text-3xl font-bold text-emerald-600">{stats.oneTimeChampions.length}</div>
